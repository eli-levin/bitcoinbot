...
//
BitcoinGuru.prototype.getPricePromise = (userId, currency, time) => {
    return new Promise((resolve, reject) => {
        let options = {
            uri: COINBASE_URI + BTC_PAIR + (currency ? currency : DEFAULT_CURRENCY) + '/spot',
            method: 'GET'
        };
        // todo: try this using x ? x : y in body above
...
//
BitcoinGuru.prototype.getTimeRangePricesPromise = (mutateDate, userId, currency) => {
    let d = new Date();
    let today = d.toISOString();
    let previousDate = mutateDate(d).toISOString();

    let p1 = this.getPricePromise(userId, currency, previousDate);
    let p2 = this.getPricePromise(userId, currency, today);
...
//
BitcoinGuru.prototype.getDailyTrendSignalPromise = (userId, currency) => {
...
//
BitcoinGuru.prototype.getWeeklyTrendSignalPromise = (userId, currency) => {
...
//
BitcoinGuru.prototype.getMonthlyTrendSignalPromise = (userId, currency) => {
...
//
BitcoinGuru.prototype.getYearlyTrendSignalPromise = (userId, currency) => {
